...
```bash
python path_planner_cli.py -m [PATH/TO/MAP] --start [START_i START_j] --goal [GOAL_i GOAL_j] --algo [bfs,dfs,astar]
```
...
```bash
python robot_plan_path.py -m [PATH/TO/MAP] --goal [GOAL_x GOAL_y] --algo [bfs,dfs,astar]
```
